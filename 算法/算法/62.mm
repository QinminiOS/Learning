...
//  算法
//
//  Created by qinmin on 2017/11/17.
...
void findSlideWindowMax(vector<int> v, int winSize)
{
    deque<int> queue;
    
    for (int i = 0; i < winSize && i < v.size(); i++) {
        if (!queue.empty() && v[queue.back()] <= v[i]) {
            queue.pop_front();
        }
    ...
    for (int i = winSize; i < v.size(); i++) {
        ...
        if (!queue.empty() && (i - winSize) >= queue.front()) {
            queue.pop_front();
        }
...
int main(int argc, const char * argv[])
{
    ...
    auto v = {2,3,4,2,6,2,5,1};
    findSlideWindowMax(v, 3);
